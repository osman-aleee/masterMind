var code = [], // Color sequence the player needs to guess
    ...
    toggelRules = document.getElementById('rules'),
    instructionsParagraph = document.getElementById('instructions-paragraph'),
    showRules = false,
    rowIncrement = 1,
    hintIncrement = 1,
    pegs = {
...
function toggleRulesInstruction () {
	if (showRules === false) {
		toggelRules.innerText = 'Hide Rules';
		instructionsParagraph.style.display = 'block';
		showRules = true;
	} else {
		toggelRules.innerText = 'Show Rules'
		instructionsParagraph.style.display = 'none'
		showRules = false
	}
}

toggelRules.addEventListener('click', toggleRulesInstruction );
